...
const $botonAgregarCasilleroSalario = document.querySelector('#agregar-casillero-salario');
$botonAgregarCasilleroSalario.onclick = function(){
    ...
    const $formularioIngreso = document.querySelector('#formulario-ingreso-salarios');
    $formularioIngreso.appendChild($inputCasilleroSalario);
    $formularioIngreso.appendChild($br);
    return false
}

const $botonEliminarCasilleroSalario = document.querySelector('#eliminar-casillero-salario')
$botonEliminarCasilleroSalario.onclick = function(){
    if (cantidadCasilleros > 1){
        let $casilleroABorrar = document.querySelector(`#salario-integrante-${cantidadCasilleros}`)
...
$botonCalcular.onclick = function(){
    ...
    document.querySelector('#mayor-salario').textContent = calcularMayorSalario(salarios);
    document.querySelector('#menor-salario').textContent = calcularMenorSalario(salarios);
    document.querySelector('#salario-anual-promedio').textContent = calcularPromedioAnual(salarios).toFixed(2);
    document.querySelector('#salario-mensual-promedio').textContent = calcularPromedioMensual(salarios).toFixed(2);
    return false
}